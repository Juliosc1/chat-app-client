      ...
      {userData.connected ? (
        <div className="bg-neutral-800 flex flex-col items-center h-screen w-2/3 rounded-2xl border-2">
          <div className="flex justify-between items-center w-[90%] border-b-2 mb-5">
            <p className="text-4xl text-white font-bold my-5 w-full">
              Chatroom
            </p>
            <p className="bg-blue-500 w-20 h-10 flex items-center justify-center text-xl text-white border-2 rounded-full mt-2">
              {userData.username}
            </p>
          </div>
          {
            <div className="flex flex-col justify-between h-full w-[90%]">
              <ul className="max-h-[70vh] w-full overflow-auto">
                {chatMessage.map((chat, index) => (
                  <li
                    className={`flex justify-end items-center h-14 gap-8 ${
                      chat.senderName === userData.username && "self"
                    }`}
                    key={index}
                  >
                    {chat.senderName !== userData.username && (
                      <div className="bg-green-400 text-white flex items-center px-4 py-1 rounded-full ">
                        {chat.message}
                      </div>
                    )}

                    {chat.senderName === userData.username && (
                      <div className="bg-emerald-600 text-white flex items-center px-4 py-1 rounded-full ">
                        {chat.message}
                      </div>
                    )}

                    {chat.senderName !== userData.username && (
                      <div className="border-2 bg-green-500 flex justify-end items-center rounded-full text-white h-10 w-auto p-1">
                        {chat.senderName}
                      </div>
                    )}

                    {chat.senderName === userData.username && (
                      <div className="border-2 bg-emerald-600 flex justify-end items-center rounded-full text-white h-10 w-auto p-1">
                        {chat.senderName}
                      </div>
                    )}
                  </li>
              ...
              <div className="bg-neutral-900 flex justify-between w-full p-2 gap-2 rounded-2xl px-5 mb-5">
                ...
                <button
                  type="button"
                  onClick={SendMessage}
                  className="bg-green-500 h-10 w-16 border-2 rounded-full text-white"
                >
                  Send
                </button>
              </div>
        ...
        <div className="bg-neutral-800 h-96 w-96 flex flex-col justify-center items-center gap-4 rounded-xl">
          ...
          <button
            onClick={connectUser}
            type="button"
            className="bg-green-600 h-10 w-28 rounded-xl text-white font-bold text-xl"
          >
            Connect
          </button>
        </div>
